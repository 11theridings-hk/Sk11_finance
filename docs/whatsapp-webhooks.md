...
| `分類` / `資金池` | 列出名稱方便入數 | 已綁定 |
| `報表` | 本月收支文字摘要 | 管理員 |
| `報表PDF` | 本月摘要 PDF（**短時下載連結**，不塞聊天檔） | 管理員 |
| `公帳 支 120 餐飲 午餐` | 預備入數 → 回覆 `確認` | 公帳成員／管理員 |
| `公帳 收 5000 租金 @公司戶` | 指定資金池 | 同上 |
| `確認` / `取消` | 確認或放棄待入數（5 分鐘內） | — |
| （口語） | 固定指令未命中時，LLM 嘗試解析成上述 Intent | 同對應權限 |

口語／提醒潤飾使用 **`OCR_API_KEY`**，模型預設 **`deepseek/deepseek-v4-pro-0813`**（與 OCR 模型分開，見 `WHATSAPP_LLM_MODEL`）。  
群組 **只收系統推播**；互動請用 1 對 1。傳圖 OCR 入數暫緩。
