...
| 指令 | 說明 | 權限 |
|------|------|------|
| `幫助` / `help` | 指令說明（跟入站語系回覆） | 已綁定用戶 |
| `提醒` / `remind` | 合約／事項／恆常到期一覽 | 合約僅管理員；事項依可見性 |
| `最近` / `recent` | 最近公帳紀錄 | 公帳成員／管理員 |
| `分類` / `categories` | 列出名稱方便入數 | 已綁定 |
| `資金池` / `pools` | 資金池列表 | 已綁定 |
| `報表` / `report` | 本月收支文字摘要 | 管理員 |
| `報表PDF` / `report pdf` | 本月摘要 PDF（**短時下載連結**；英文指令→英文 PDF） | 管理員 |
| `公帳 支 120 …` / `ledger expense 120 …` | 預備入數 → `確認`／`confirm` | 公帳成員／管理員 |
| `確認` / `confirm` · `取消` / `cancel` | 確認或放棄待入數（5 分鐘內） | — |
| （口語中／英） | 固定指令未命中時，LLM 嘗試解析；回覆語系跟入站 | 同對應權限 |

口語／提醒潤飾使用 **`OCR_API_KEY`**，模型預設 **`deepseek/deepseek-v4-pro-0813`**（與 OCR 模型分開）。  
**英文入站 → 英文回覆／英文版 PDF**；中文入站 → 中文。群組只收推播。傳圖 OCR 入數暫緩。

完整會計結算包仍建議網頁匯出；WhatsApp PDF 為本月摘要捷徑。
